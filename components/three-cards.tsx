...
export default function ThreeCards({ heading, para, isBlack = false, features }: { heading: string; para: string | null, isBlack?: boolean, features: any[] }) {
    ...
    return (
        <section className={`relative w-full ${isBlack ? 'bg-[#161616]' : 'bg-[#F7F7F7]'} py-8 md:py-16 overflow-hidden`}>
            ...
            <div className="2xl:max-w-360 w-[90%] mx-auto relative z-10 flex flex-col items-center">
                {/* Heading */}
                <motion.h2
                ...
                <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full">
                    {features.map((feature, idx) => (
                        <motion.div
                            key={idx}
                            initial={{ opacity: 0, y: 30 }}
