...
export default function Navbar({ settings }: { settings: NavbarSettings }) {
  ...
  const [dropdownImage, setDropdownImage] = useState(
    settings.solutionItems[0]?.hoverImage ?? ""
  );
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (
        ...
        setDropdownImage(settings.solutionItems[0]?.hoverImage ?? "");
      }
    };
    ...
    <nav
      className={cn(
        ...
        isMenuOpen
          ? "bg-[#0A0A0A]"
          : isScrolled
            ? "bg-[#0A0A0A] lg:bg-black/20 lg:backdrop-blur-md"
      ...
      <div className=" text-white py-2 ">
        <div className="max-w-360 w-[90%] mx-auto  flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-10 font-onest text-[15px]">
            <Link href="/" className="hover:text-[#168DCA] transition-colors">
              Home
            </Link>
            <Link
              href="/about"
              className="hover:text-[#168DCA] transition-colors"
            >
              About Us
            </Link>
                    ...
                    <div className="flex">
                      ...
                      <div className="flex-1 p-5">
                        <ul className="space-y-3">
                          {settings.solutionItems.map((item, i) => (
                            <li key={i}>
                              <Link
                                href={item.href}
                                onClick={() => setIsSolutionsOpen(false)}
                                onMouseEnter={() =>
                                  setDropdownImage(item.hoverImage)
                                }
            ...
            <div className="flex flex-col h-full p-6 pt-10">
              <div className="flex justify-between items-center mb-12">
                ...
                <button
                  ...
                  className="p-3 border border-white/10 rounded-full hover:bg-white/5 transition-colors text-white"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-6 text-xl font-medium text-white">
                ...
                <div className="space-y-4">
                  <div className="hover:text-[#168DCA] transition-colors">
                    Solutions
                  </div>
                ...
                <Link
                  href="/contact"
                  onClick={() => setIsMenuOpen(false)}
                  className="hover:text-[#168DCA] transition-colors text-white"
                >
                  Contact Us
                </Link>
              </div>
              ...
              <div className="mt-auto pt-10 border-t border-white/10 space-y-8 pb-10">
                <div className="flex flex-col gap-5 text-sm text-white/70">
                  <div className="flex items-center gap-4 group">
                    <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-[#168DCA]/10 transition-colors">
                      <Mail size={18} className="text-[#168DCA]" />
                    </div>
                    {settings.contactEmail}
                  </div>
                  <div className="flex items-center gap-4 group">
                    <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-[#168DCA]/10 transition-colors">
                      <Phone size={18} className="text-[#168DCA]" />
                    </div>
                    {settings.contactPhone}
                  </div>
                </div>
                <Link
                  href={settings.primaryCtaHref}
                  onClick={() => setIsMenuOpen(false)}
                  className="bg-white text-black w-full py-3 rounded-xl text-center font-bold block hover:bg-gray-200 transition-colors shadow-xl"
                >
                  {settings.primaryCtaLabel}
                </Link>
              </div>
