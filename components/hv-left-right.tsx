...
export default function HvLeftRight({
  sections,
  isBlack = false,
}: SplitSectionProps) {
  const CheckIcon = () => (
  ...
  return (
    <section className={`${isBlack ? 'bg-[#161616]' : ''} space-y-8 md:space-y-20 py-8 md:py-20`}>
          ...
          <div className={`${section.reverse ? "lg:order-2" : ""}`}>
            ...
            <div className="space-y-3 md:space-y-3 pt-2 mb-4">
              {section.paragraphs.map((para, i) => (
                <p
                  key={i}
                  className={`text-[14px] md:text-[15px] leading-relaxed ${isBlack ? 'text-white' : 'text-[#333333]'}`}
                >
                  {para}
                </p>
              ))}
            </div>
              <p className={`text-[16px] md:text-[18px] font-medium ${isBlack ? 'text-white' : 'text-black'}`}>
              {section.keytitle}
            </p>
